...
void Timer0IntHandler(void) {
    ...
    if(ADCtoggle == 1) {
      ...
      GrStringDraw(&sContext, buf, -1, 40, 46, 1);
    }
...
void processMenu(int32_t input) {  
  ...
  if (input == 'A') {
    ...
    if(registerToggle) {
        IntRegister(INT_TIMER1A, isr_asm_start);
         UARTSend("Switched to Assembly\n",21);
    }
    else {
        IntRegister(INT_TIMER1A, Timer1IntHandler);
        UARTSend("Switched to C\n",14);
    }
  }
...
int main(void) {
    ...
    //SET THE CLOCKING TO RUN AT 16MHZ
    ...
    GrStringDraw(&sContext, "ADC:", -1, 16, 46, 0);
